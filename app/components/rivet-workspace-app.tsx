...
function GlobalGuideSearch({ workspaceId, guides, onOpen }: {
  workspaceId: string;
  guides: Guide[];
  onOpen: (guide: Guide) => void;
}) {
  ...
  function openResult(result: GuideSearchResult) {
    ...
    // A result visible to search is visible to the list; fall back to the
    // server-authorized read by navigating to the workspace guide view.
    const params = new URLSearchParams({ workspaceId, guide: result.guideId });
  ...
  return (
    <div className="app-shell" style={{ "--workspace-accent": workspace.settings.accentColor, "--click-color": workspace.settings.clickTargetColor } as React.CSSProperties}>
      <aside className={`sidebar${navOpen ? " sidebar-open" : ""}`}>
        ...
        <nav className="main-nav" aria-label="Main navigation">{visibleNav.map(({ view: item, icon: Icon }) => <button className={view === item ? "active" : ""} type="button" key={item} onClick={() => { setView(item); setNavOpen(false); }}><Icon /><span>{item}</span>{item === "Guides" && active.metrics.reviews ? <small>{active.metrics.reviews}</small> : null}{item === "Members" && pendingSupportCount ? <small className="nav-badge">{pendingSupportCount}</small> : null}</button>)}</nav>
