...
function MfaEnrollmentInitialView({
  busy,
  error,
  onBegin,
}: {
...
function MfaEnrollmentRecoveryCodesView({
  busy,
  recoveryCodes,
  onAcknowledge,
}: {
  ...
  return (
    <>
      ...
      >
        {busy ? <LoaderCircle className="auth-spin" /> : <CheckCircle2 />}{" "}
        Continue and verify
      </button>
    </>
  );
...
function MfaEnrollmentSecretView({
  busy,
  error,
  secret,
  qrCodeDataUrl,
      ...
      <section className="auth-panel" aria-labelledby="mfa-enroll-heading">
        <div className="auth-card">
          {!secret && !recoveryCodes ? (
            <MfaEnrollmentInitialView
              busy={busy}
              error={error}
              onBegin={onBegin}
            />
          ) : recoveryCodes ? (
            <MfaEnrollmentRecoveryCodesView
              busy={busy}
              recoveryCodes={recoveryCodes}
              onAcknowledge={onAcknowledge}
            />
          ) : (
            <MfaEnrollmentSecretView
              busy={busy}
              error={error}
              secret={secret}
