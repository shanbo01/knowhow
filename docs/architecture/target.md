...
The checked-in Appwrite resource manifest is endpoint-portable. The intended control plane is self-hosted Appwrite 1.9.6 on the Bicep-managed Azure VM. The private-beta test instance uses South India while Qatar Central remains the later parameterized target. Production and Staging use separate project IDs, keys, users, database rows, Storage objects, Sites, and Function deployments. Appwrite Cloud Frankfurt is retained only as a synthetic/non-sensitive fallback until Azure parity, backup, restore, and journey gates pass; it is then removed from runtime configuration.
