...
Status: Qatar execution pending Azure VM allocation. The intended self-hosted platform uses daily locally redundant Azure VM recovery points plus age-encrypted application-consistent Appwrite volume, MongoDB, and configuration archives in private versioned zone-redundant Qatar Blob storage; see `infrastructure/azure/README.md`. The database and restored-application HMAC evidence contracts below remain the application-level acceptance gate, but Cloud Pro-specific policy/archive API steps apply only to the temporary Frankfurt fallback. The pilot target is RPO up to 24 hours and best-effort RTO within one business day; these are internal targets, not a contractual SLA.
